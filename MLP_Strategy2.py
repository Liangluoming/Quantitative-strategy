...
for test_year in range(2013, 2024, 1):
    ...
    val_criterion = NegativeRankICLoss() ## Rank IC 损失函数
    ...
    """
    损失函数的设置参照长江证券关于TCN模型的报告
    Loss1:考虑单个因子的IC
    Loss2:考虑和合成因子的IC
    Loss3:考虑因子间的相关性
    """

    ones_matrix = torch.diag(torch.ones(output_size)).to(device)
    ...
    for epoch in range(num_epochs):
        ...
        for batch_X, batch_y in train_loader:
            ...
            loss3 = torch.corrcoef(factors.view(output_size, -1)) - ones_matrix
            loss3 = torch.square(torch.max(loss3)) * output_size**2 
            factor_compound = factors.mean(dim=1, keepdim=True)
            loss2 = criterion2(factor_compound, batch_y)
            batch_y = batch_y.repeat(1, output_size)
            loss1 = criterion2(factors, batch_y)
            loss = 2 * loss1 + 2 * loss2 + 1 * loss3
...
test_result.to_parquet("/home/lenovo/liangluoming/results/llm_mlp_1_rolling9_L2.parquet",index=False)